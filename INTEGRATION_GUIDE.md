...
KeenpleShell.createTurnBased({
  ...
  modes: {
    mp: {
      ...
      matchVariant: 'full',                       // Keenple.Match 큐 키 분리 (머신 id)
      matchVariantLabel: { ko: '완전예측', en: 'Full Prediction' },  // 대기 오버레이 UX 라벨 (v2.26.0+)
    },
  },
  ...
});
...
### 12.8 matchVariantLabel — 대기 오버레이 UX 라벨 (v2.26.0+)

위 §12.1 네 옵션은 전부 **분리(큐·방 리스트)** 담당이고 머신 id (`matchVariant`) 를 사용합니다. `matchVariantLabel` 은 분리와 무관한 **표시** 옵션으로, `Keenple.Match` SDK 가 그리는 "상대를 찾는 중..." 대기 오버레이 앞에 variant 이름을 붙여 사용자에게 보여주기 위한 i18n 라벨입니다.

**합성 형식** (SDK 주관, main 커밋 `8ca377f`):

```
[matchVariantLabel.ko] 상대를 찾는 중...
[matchVariantLabel.en] Finding opponent...
```

예 — 장기 완전예측:
```js
modes.mp.matchVariant: 'full',
modes.mp.matchVariantLabel: { ko: '완전예측', en: 'Full Prediction' },
// 오버레이: "[완전예측] 상대를 찾는 중..." / "[Full Prediction] Finding opponent..."
```

**언제 쓰는가**: 같은 gameKey 아래 여러 variant 큐를 동시 운영할 때, 사용자가 자기가 어느 큐에서 대기 중인지 인지 못 해서 "왜 매칭이 안 되지?" 로 느끼는 케이스 방지.

**미선언 시**: SDK 는 기본 `"상대를 찾는 중..."` 만 표시 — 단일 variant 게임이나 label 이 불필요한 경우 생략해도 동작.

**validateConfig**: 선언 시 `{ ko, en }` 문자열 객체 강제. 위반 시 throw.

**주의**: `matchVariant` (머신 id, `^[a-z0-9_-]{1,32}$`) 와 `matchVariantLabel` (i18n 표시 텍스트) 은 별개 필드. 한쪽만 선언해도 되지만 실제 가치는 둘 다 선언할 때 극대화됨.
